...
export default function Form({ children, onSubmit, className = '' }) {
  ...
  const adjustInputWidth = (input) => {
    if (input && input.value) {
      const minWidth = input.placeholder.length; 
      if (input.value.length < minWidth) {
        input.style.width = `${minWidth + 1}ch`;
      } else {
  ...
  const wrappedFormContent = (child) => React.Children.map(child?.props.children, (inputChild, index) => {
    if (React.isValidElement(inputChild) && inputChild.type === 'input') {
      return <div className='container-input' key={index}>{inputChild}</div>;
    }
    return inputChild;
  });
  ...
  return (
    <form className={`form ${className}`} onSubmit={onSubmit}>
      <svg id='fingerprint' xmlns="http://www.w3.org/2000/svg" fill="#000000" viewBox="0 0 24 24">
